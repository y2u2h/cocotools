...
def make_hist(annotation, categories, images, model_limit_width, model_limit_height, csv):
    cocoGt = COCO(annotation)

    catids = [cid for cid in cocoGt.cats]
    if categories:
        catids = []
    ...
    imgids = [iid for iid in cocoGt.imgs]
    if images:
        imgids = []
    ...
    for k, v in cocoGt.anns.items():
        if v["category_id"] in catids:
            if v["image_id"] in imgids:
                if csv is True and first is True:
                    ...
                    if model_limit_width != 0 and model_limit_height != 0:
                        print(
                ...
                if model_limit_width != 0 and model_limit_height != 0:
                    mag_x = float(model_limit_width) / float(iwidth)
                    mag_y = float(model_limit_height) / float(iheight)
                    mag = min(mag_x, mag_y)
...
def main():
    ...
    parser.add_argument("-model_limit_width", "--model_limit_width", default=0)
    parser.add_argument("-model_limit_height", "--model_limit_height", default=0)
    ...
    make_hist(
        ...
        args.categories,
        args.images,
        args.model_limit_width,
        args.model_limit_height,
        args.csv,
    )
